...
type
  ...
  THashTigerClass = class of THashTiger;
...
function THashTiger.SelfTest: boolean;
const
...
var
  TestOut: array[0..2 * SizeOf(Int64)] of Byte;
begin
  Init;
  ...
  Init;
  UpdateStr('Tiger - A Fast New Hash Function, by Ross Anderson and Eli Biham');
  Final(TestOut);
...
begin
  Burn;
  fCurHash[0]:= $0123456789ABCDEF;
  fCurHash[1]:= $FEDCBA9876543210;
  fCurHash[2]:= $F096A5B4C3B2E187;
end;

procedure THashTiger.Update(const Buffer; Size: Cardinal);
var
  PBuf: ^byte;
begin
  Inc(fLen,Size*8);
...
procedure THashTiger.Final(var Digest: array of Byte);
begin
  fHashBuff[fIndex]:= $01;
  if fIndex>= 56 then
    Compress;
  Pint64(@fHashBuff[56])^:= fLen;
  Compress;
...
initialization
  THash.RegisterHashType('Tiger', THashTiger, SizeOf(Int64) * 3);
